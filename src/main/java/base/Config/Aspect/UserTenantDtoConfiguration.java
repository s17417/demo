...
public class UserTenantDtoConfiguration implements IDtoConfigurtion {
	...
	mapper
	.createTypeMap(UsersTenantRole.class, UserTenantRoleDTO.class)
	.addMappings(map -> {
	...
	mapper
	.createTypeMap(UserTenantRoleDTO.class, UsersTenantRole.class, DTOObjectConstans.UPDATE.name())
	.addMappings(map ->{
	...
	mapper.createTypeMap(UsersTenantRole.class, UserSignedTenantDTO.class)
	.addMappings(map -> {
		map.<String>map(src -> src.getUser().getFirstname(), (dest,v) -> dest.setFirstname(v));
		map.<String>map(src -> src.getUser().getSurname(), (dest,v) -> dest.setSurname(v));
	}).implicitMappings();
